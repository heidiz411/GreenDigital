...
class ReportController extends Controller
{
    ...
    {
        // simple inline view (can be moved into a dedicated view later)
    ...
    {
        Auth::verifyCsrf();
        $txt = trim((string)$this->input('rep_content', ''));
        if ($txt === '') $this->json(['success' => false, 'message' => 'กรุณาระบุรายละเอียด'], 422);
        $id = (new Report())->create(['rep_content' => $txt, 'user_id' => Auth::id()]);
        $this->json(['success' => true, 'message' => 'ส่งรายงานเรียบร้อย', 'id' => $id]);
    }
}
