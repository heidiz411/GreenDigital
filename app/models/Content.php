...
class Content extends Model
{
    ...
    {
        $st = $this->db->prepare("SELECT * FROM {$this->table} WHERE content_id=?");
        $st->execute([$id]);
    ...
    {
        $st = $this->db->prepare("DELETE FROM {$this->table} WHERE content_id=?");
        return $st->execute([$id]);
    }

    public function listAdmin(): array
    {
        return $this->db->query("SELECT content_id,title,publish_at,expire_at,is_active FROM {$this->table} ORDER BY content_id DESC")->fetchAll();
    }
}
